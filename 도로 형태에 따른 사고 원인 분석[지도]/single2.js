d3.csv("data2.csv", function(data) {
  ...
  var filtered1 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반"
  })
  ...
  var filtered2 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "강원"
  })
  var filtered3 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "경기"
  })
  var filtered4 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "경남"
  })
  var filtered5 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "경북"
  })
  var filtered6 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "광주"
  })
  var filtered7 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "대구"
  })
  var filtered8 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "대전"
  })
  var filtered9 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "부산"
  })
  var filtered10 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "서울"
  })
  var filtered11 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "세종"
  })
  var filtered12 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "울산"
  })
  var filtered13 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "인천"
  })
  var filtered14 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "전남"
  })
  var filtered15 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "전북"
  })
  var filtered16 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "제주"
  })
  var filtered17 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "충남"
  })
  var filtered18 = data.filter(function(d) {
    return d["도로형태_대분류"] == "단일로" && d["법규위반"] == "신호위반" && d["발생지시도"] == "충북"
  })
  ...
  var txt = "단일로에서의 신호위반 수 : " + filtered1.length + " (" + (Math.round(filtered1.length / filtered.length * 1000) / 10) + "%" + ")";
  svg.append("text")
    .attr("x", 600)
    .attr("y", 50)
