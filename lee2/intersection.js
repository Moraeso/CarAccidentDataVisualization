d3.csv("data2.csv", function(data) {
  ...
  var filtered1 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행"
  })
  ...
  var filtered2 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "강원"
  })
  var filtered3 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "경기"
  })
  var filtered4 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "경남"
  })
  var filtered5 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "경북"
  })
  var filtered6 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "광주"
  })
  var filtered7 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "대구"
  })
  var filtered8 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "대전"
  })
  var filtered9 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "부산"
  })
  var filtered10 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "서울"
  })
  var filtered11 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "세종"
  })
  var filtered12 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "울산"
  })
  var filtered13 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "인천"
  })
  var filtered14 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "전남"
  })
  var filtered15 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "전북"
  })
  var filtered16 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "제주"
  })
  var filtered17 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "충남"
  })
  var filtered18 = data.filter(function(d) {
    return d["도로형태_대분류"] == "교차로" && d["법규위반"] == "안전운전 의무 불이행" && d["발생지시도"] == "충북"
  })
  ...
  var txt = "교차로에서의 안전운전 의무 불이행 수 : " + filtered1.length + " (" + (Math.round(filtered1.length / filtered.length * 1000) / 10) + "%" + ")";
  svg.append("text")
    .attr("x", 600)
    .attr("y", 50)
