d3.csv("./data2.csv", function(error, data) {
  ...
  dataSet[0] = (Math.round(dataSet2[0]/d3.sum(dataSet2)*1000)/10);
  ...
  dataSet[1] = (Math.round(dataSet2[0]/d3.sum(dataSet2)*1000)/10);
  ...
  barElements.enter()
    ...
    .attr("y", function(d, i) { //Y 좌표를 지정
      return svgHeight - d*5.5 - offsetY; //Y 좌표를 계산
    })
    .attr("height", function(d, i) { //넓이설정.2번째의 파라미터에 함수지정
      return d*5.5; //데이터 값을 그대로 높이로 지정
    })
  ...
  barElements.enter()
    ...
    .range([dataMax, -dataMax*4.5]); //실제 출력 크기
